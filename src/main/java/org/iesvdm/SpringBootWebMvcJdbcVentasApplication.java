...
public class SpringBootWebMvcJdbcVentasApplication implements CommandLineRunner{
	...
	public void run(String... args) throws Exception {
		...
		int identifier = 2;
		Optional<Comercial> comercial = comercialDAO.find(identifier);

		if (comercial.isPresent()) {
			log.info("Cliente {}: {}", identifier, comercial.get());
			...
			comercial = comercialDAO.find(identifier);

			log.info("Cliente {}: {}", identifier, cliente.get());
